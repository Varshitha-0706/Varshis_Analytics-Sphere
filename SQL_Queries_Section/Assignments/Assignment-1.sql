...
select Department,sum(SaleAmount)[Total_Sales] from EmployeeSales group by Department

--2)Write a query to count the number of sales made by each employee.
select EmployeeID,count(*)[No_of_Sales] from EmployeeSales group by EmployeeID

--3)Write a query to calculate the average sale amount for each department.
select Department,AVG(SaleAmount)[Sales_Average] from EmployeeSales group by Department
...
select EmployeeID,SUM(SaleAmount)[Total_Sales] from EmployeeSales group by EmployeeID HAVING COUNT(SaleID)>1
